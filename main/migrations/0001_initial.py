# Generated by Django 5.2.8 on 2025-11-12 14:33

import django.db.models.deletion
from django.conf import settings
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Product',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=200, verbose_name='Name')),
                ('ref', models.CharField(max_length=50, unique=True, verbose_name='Reference/SKU')),
                ('price', models.DecimalField(decimal_places=2, max_digits=6, verbose_name='Price')),
                ('flavor', models.CharField(blank=True, max_length=100, null=True, verbose_name='Flavor')),
                ('size', models.CharField(blank=True, max_length=100, null=True, verbose_name='Size')),
                ('image', models.ImageField(blank=True, null=True, upload_to='products')),
            ],
            options={
                'verbose_name': 'Product',
                'verbose_name_plural': 'Products',
            },
        ),
        migrations.CreateModel(
            name='Testimonial',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('author', models.CharField(max_length=500, verbose_name='Author')),
                ('text', models.TextField(verbose_name='Testimonial Text')),
                ('photo', models.ImageField(blank=True, null=True, upload_to='testimonials/', verbose_name='Client Photo')),
                ('date_published', models.DateTimeField(auto_now_add=True)),
            ],
            options={
                'verbose_name': 'Testimonial',
                'verbose_name_plural': 'Testimonials',
            },
        ),
        migrations.CreateModel(
            name='ProductSolicitation',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('solicitant_name', models.CharField(max_length=255, verbose_name='Solicitant Full Name (String)')),
                ('solicitant_contact', models.CharField(max_length=255, verbose_name='Solicitant Contact (Email/Phone)')),
                ('quantity', models.PositiveIntegerField(default=1, verbose_name='Quantity')),
                ('date', models.DateTimeField(auto_now_add=True, verbose_name='Date')),
                ('is_paid', models.BooleanField(default=False, verbose_name='Is Paid (estaPagado)')),
                ('status', models.CharField(choices=[('SOLICITADO', 'SOLICITED'), ('ENCARGADO', 'ORDERED'), ('RECIBIDO_NATURSUR', 'RECEIVED BY NATURSUR'), ('RECOGIDO_CLIENTE', 'PICKED UP BY CLIENT')], default='SOLICITADO', max_length=20, verbose_name='Status (estado)')),
                ('product', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, to='main.product', verbose_name='Product (id_producto)')),
                ('registered_user', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, to=settings.AUTH_USER_MODEL, verbose_name='Linked Registered User')),
            ],
            options={
                'verbose_name': 'Solicited Product',
                'verbose_name_plural': 'Solicited Products',
            },
        ),
        migrations.CreateModel(
            name='UserProfile',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('phone', models.CharField(blank=True, max_length=20, null=True, verbose_name='Phone')),
                ('photo', models.ImageField(blank=True, null=True, upload_to='user_photos/', verbose_name='Photo')),
                ('user', models.OneToOneField(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'verbose_name': 'User Profile',
                'verbose_name_plural': 'User Profiles',
            },
        ),
    ]
